...
std::string TransportCatalog::RenderMap() const {
    ...
    vector<std::pair<double, std::string>> lons, lats;

    for (const auto& [name, stop] : stops_) {
        lons.emplace_back(stop.position.longitude, name);
