...
struct AddressHasher {
    size_t operator() (const Address& adress) const {
        return coef * coef * str_hash(adress.city) +
                      coef * str_hash(adress.street) +
                             int_hash(adress.building);
    }

    const size_t coef = (1 << 16) - 1; // = 2^32 - 1
...
struct PersonHasher {
    size_t operator() (const Person& person) const {
        return coef * coef * coef * str_hash(person.name) +
                      coef * coef * adress_hash(person.address) +
                             coef * int_hash(person.height) +
                                    double_hash(person.weight);
    }

    const size_t coef = (1 << 16) - 1; // = 2^32 - 1

    AddressHasher   adress_hash;
    hash<string>    str_hash;
    hash<int>       int_hash;
