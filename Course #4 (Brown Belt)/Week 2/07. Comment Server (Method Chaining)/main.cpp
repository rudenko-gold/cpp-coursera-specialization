...
public:
    ...
    friend ostream& operator << (ostream& output, const HttpResponse& resp) {
        output << "HTTP/1.1 " << (int)resp.httpCode << " ";
        ...
        for (auto& [head, head_value] : resp.headers) {
            output << head << ": " << head_value << endl;
        }

        if (resp.a_content.size() > 0) {
            output << "Content-Length: " << to_string(resp.a_content.size()) << endl;
        }
        
        output << endl;

        if (resp.a_content.size() > 0) {
            output << resp.a_content;
        }
