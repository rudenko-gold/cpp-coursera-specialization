...
public:
    ...
    public:
        ...
        long double outcome = 0.0;

        long double get() {
            return profit - outcome;
        }

        void payTax(int persentage) {
            profit *= (1.0 - (persentage / 100.0));
        }
    };
    ...
    long double computeIncome(const Date& from, const Date& to) const {
        ...
        for (size_t day = dayFrom; day <= dayTo; ++day) {
            totalIncome.profit += income[day].profit;
            totalIncome.outcome += income[day].outcome;
        }

        return totalIncome.get();
    }

    void earn(const Date& from, const Date& to, long double valueForAllDays) {
        const size_t dayFrom = ParseDate(from.toString());
        const size_t dayTo = ParseDate(to.toString());

        long double valuePerDay = valueForAllDays / (dayTo - dayFrom + 1);
        for (size_t day = dayFrom; day <= dayTo; ++day) {
            income[day].profit += valuePerDay;
        }
    }

    void spend(const Date& from, const Date& to, long double valueForAllDays) {
        const size_t dayFrom = ParseDate(from.toString());
        const size_t dayTo = ParseDate(to.toString());

        long double valuePerDay = valueForAllDays / (dayTo - dayFrom + 1);
        for (size_t day = dayFrom; day <= dayTo; ++day) {
            income[day].outcome += valuePerDay;
        }
    }
    ...
    void payTax(Date from, Date to, int persentage = 13) {
        ...
        for (size_t day = dayFrom; day <= dayTo; ++day) {
            income[day].profit *= (1.0 - (persentage / 100.0));
        }
    }
...
int main() {
    ...
    for (size_t query = 0; query < queriesSize; ++query) {
        ...
        if (command == "Earn") {
            ...
            int persentage;
            cin >> persentage;
            budgetManager.payTax(Date(fromDate), Date(toDate), persentage);
        } else if (command == "Spend") {
            double value;
            cin >> value;
            budgetManager.spend(Date(fromDate), Date(toDate), value);
        }
    }
