...
public:
    ...
    WriteAccess operator[](const K& key) {
        ...
        return { lock_guard(mutexes[bucketId]), buckets[bucketId][key] };
    }

    ReadAccess At(const K& key) const {
        size_t bucketId = GetBucketId(key);
        return { lock_guard(mutexes[bucketId]), buckets[bucketId].at(key) };
    }

    bool Has(const K& key) const {
        size_t bucketId = GetBucketId(key);
        return buckets[bucketId].find(key) != buckets[bucketId].end();
    }
    ...
    MapType BuildOrdinaryMap() const {
        ...
        for (size_t bucketId = 0; bucketId < buckets.size(); ++bucketId) {
            lock_guard<mutex> guard(mutexes[bucketId]);
            resultMap.insert(buckets[bucketId].begin(), buckets[bucketId].end());
        }
